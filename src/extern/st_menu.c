 ...
 */
void st_menu_save(struct ST_MENU *menu, int *cursor_rows, int **refvals, int max_items) {
 ...
 */
void st_menu_load(struct ST_MENU *menu, int *cursor_rows, int **refvals) {
        ...
        if (*menu_items->text && strncmp(menu_items->text, "--", 2) != 0) {
            ...
            if (config->left_alligned_shortcuts) {
                ...
                *columns = max_int(*columns,
                                   1 + text_width + 1 + (config->extra_inner_space ? 2 : 0) + (shortcut_width > 0 ? shortcut_width + 4 : 0));
        }
 ...
 * should be panelized.
 */
void st_menu_set_desktop_window(WINDOW *win) {
    desktop_win = win;
}
 ...
 */
void st_menu_post(struct ST_MENU *menu) {
    curs_set(0);
    noecho();
 ...
 */
void st_menu_set_focus(struct ST_MENU *menu, int focus) {
    menu->focus = focus;
}
...
_st_menu_driver(struct ST_MENU *menu, int c, bool alt, MEVENT *mevent,
    ...
    *unpost_submenu = false;
     ...
     */
    if (c != KEY_MOUSE && c != KEY_HOME && c != KEY_END && c != KEY_UP && c != KEY_DOWN && c != KEY_LEFT && c != KEY_RIGHT) {
...
bool st_menu_driver(struct ST_MENU *menu, int c, bool alt, MEVENT *mevent) {
        ...
        if (menu_item->submenu) {
            menu->submenus[i] =
                st_menu_new(config, menu_item->submenu,
                            begin_y + i + config->submenu_offset_y + (config->draw_box ? 1 : 0) + (config->wide_vborders ? 1 : 0),
        ...
        if (menu_item->submenu) {
            menu->submenus[i] =
                st_menu_new(pdcfg, menu_item->submenu,
                            1, menu->bar_fields_x_pos[i] + pdcfg->menu_bar_menu_offset - (pdcfg->draw_box ? 1 : 0) - (pdcfg->wide_vborders ? 1 : 0) - (pdcfg->extra_inner_space ? 1 : 0) - 1, NULL);
        } else
            menu->submenus[i] = NULL;
...
void st_menu_free(struct ST_MENU *menu) {
    _st_menu_free(menu);
 ...
 */
bool st_menu_enable_option(struct ST_MENU *menu, int code, int option) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
    int i = 0;
 ...
 */
bool st_menu_reset_option(struct ST_MENU *menu, int code, int option) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
    int i = 0;
 ...
 * Reset flag of first menu item specified by code
 */
bool st_menu_reset_all_options(struct ST_MENU *menu, int option) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
    int i = 0;
 ...
 * Reset flag of first menu item specified by code
 */
bool st_menu_reset_all_submenu_options(struct ST_MENU *menu, int menu_code, int option) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
    int i = 0;
 ...
 * Set flag of first menu item specified by code
 */
bool st_menu_set_option(struct ST_MENU *menu, int code, int option, bool value) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
    int i = 0;
...
void st_cmdbar_post(struct ST_CMDBAR *cmdbar) {
    active_cmdbar = cmdbar;
    cmdbar_draw(cmdbar);
}

void st_cmdbar_unpost(struct ST_CMDBAR *cmdbar) {
    active_cmdbar = NULL;
 ...
 */
bool st_menu_set_ref_option(struct ST_MENU *menu,
                            int code,
                            int option,
                            int *refvalue) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
 ...
 */
bool st_menu_set_shortcut(struct ST_MENU *menu, int code, char *shortcut) {
    ST_MENU_ITEM *menu_items = menu->menu_items;
    int i = 0;
 ...
 */
int st_menu_get_focus(struct ST_MENU *menu) {
    return menu->focus;
}
