...
constexpr bool CanDeferToLastStep(const DocModification &mh) noexcept {
  ...
  if (mh.modificationType & SC_MULTISTEPUNDOREDO) return true; // CAN skip
  return false;                                                // PRESUMABLY must do
}

constexpr bool CanEliminate(const DocModification &mh) noexcept {
  return (mh.modificationType & (SC_MOD_BEFOREINSERT | SC_MOD_BEFOREDELETE)) != 0;
}
...
Timer::Timer() noexcept : ticking(false), ticksToWait(0), tickerID{} {}

Idler::Idler() noexcept : state(false), idlerID(0) {}
...
static constexpr bool IsAllSpacesOrTabs(std::string_view sv) noexcept {
  for (const char ch : sv) {
    ...
    if (!IsSpaceOrTab(ch)) return false;
  }
  return true;
}
...
void Editor::SetRepresentations() {
  ...
  // C0 control set
  const char *const reps[] = {"NUL", "SOH", "STX", "ETX", "EOT", "ENQ", "ACK", "BEL",
  ...
  if (IsUnicodeMode()) {
    const char *const repsC1[] = {"PAD", "HOP",  "BPH", "NBH", "IND", "NEL", "SSA", "ESA",
...
SelectionPosition Editor::ClampPositionIntoDocument(SelectionPosition sp) const {
  if (sp.Position() < 0) {
    ...
    if (!pdoc->IsLineEndPosition(sp.Position())) sp.SetVirtualSpace(0);
    return sp;
  }
}
...
SelectionPosition Editor::SPositionFromLocation(Point pt, bool canReturnInvalid,
                                                bool charPosition, bool virtualSpace) {
  RefreshStyleData();
  AutoSurface surface(this);
  ...
  if (canReturnInvalid) {
    if (!rcClient.Contains(pt)) return SelectionPosition(INVALID_POSITION);
    if (pt.x < vs.textStart) return SelectionPosition(INVALID_POSITION);
    if (pt.y < 0) return SelectionPosition(INVALID_POSITION);
  }
  ...
  return view.SPositionFromLocation(surface, *this, ptdoc, canReturnInvalid, charPosition,
                                    virtualSpace, vs, rcClient);
}

Sci::Position Editor::PositionFromLocation(Point pt, bool canReturnInvalid,
                                           bool charPosition) {
  return SPositionFromLocation(pt, canReturnInvalid, charPosition, false).Position();
}

/**
 * Find the document position corresponding to an x coordinate on a particular document
 * line. Ensure is between whole characters when document is in multi-byte or UTF-8 mode.
...
SelectionPosition Editor::SPositionFromLineX(Sci::Line lineDoc, int x) {
  RefreshStyleData();
  if (lineDoc >= pdoc->LinesTotal()) return SelectionPosition(pdoc->Length());
...
void Editor::RedrawRect(PRectangle rc) {
  ...
  const PRectangle rcClient = GetClientRectangle();
  if (rc.top < rcClient.top) rc.top = rcClient.top;
  if (rc.bottom > rcClient.bottom) rc.bottom = rcClient.bottom;
  if (rc.left < rcClient.left) rc.left = rcClient.left;
...
void Editor::Redraw() {
  ...
  if (wMargin.GetID()) wMargin.InvalidateAll();
  // wMain.InvalidateAll();
}
...
void Editor::RedrawSelMargin(Sci::Line line, bool allAfter) {
  ...
  if (!wMargin.GetID() ||
      markersInText) { // May affect text area so may need to abandon and retry
    if (AbandonPaint()) {
      return;
    }
  }
  ...
  if (line != -1) {
    ...
    if (vs.largestMarkerHeight > vs.lineHeight) {
      ...
      rcLine.top -= delta;
      rcLine.bottom += delta;
      if (rcLine.top < rcMarkers.top) rcLine.top = rcMarkers.top;
      if (rcLine.bottom > rcMarkers.bottom) rcLine.bottom = rcMarkers.bottom;
    }

    rcMarkers.top = rcLine.top;
    if (!allAfter) rcMarkers.bottom = rcLine.bottom;
    if (rcMarkers.Empty()) return;
  }
  if (wMargin.GetID()) {
    const Point ptOrigin = GetVisibleOriginInMain();
...
PRectangle Editor::RectangleFromRange(Range r, int overlap) {
  const Sci::Line minLine = pcs->DisplayFromDoc(pdoc->SciLineFromPosition(r.First()));
  const Sci::Line maxLine = pcs->DisplayLastFromDoc(pdoc->SciLineFromPosition(r.Last()));
  ...
  if (rc.top < rcClientDrawing.top) rc.top = rcClientDrawing.top;
...
void Editor::SetRectangularRange() {
  if (sel.IsRectangular()) {
    ...
    AutoSurface surface(this);
    for (Sci::Line line = lineAnchorRect; line != lineCaret + increment;
         line += increment) {
      SelectionRange range(view.SPositionFromLineX(surface, *this, line, xCaret, vs),
                           view.SPositionFromLineX(surface, *this, line, xAnchor, vs));
      if ((virtualSpaceOptions & SCVS_RECTANGULARSELECTION) == 0) range.ClearVirtualSpace();
...
void Editor::ThinRectangularRange() {
  if (sel.IsRectangular()) {
    ...
    if (sel.Rectangular().caret < sel.Rectangular().anchor) {
      sel.Rectangular() =
          SelectionRange(sel.Range(sel.Count() - 1).caret, sel.Range(0).anchor);
    } else {
      sel.Rectangular() =
          SelectionRange(sel.Range(sel.Count() - 1).anchor, sel.Range(0).caret);
    }
    SetRectangularRange();
  }
}
...
void Editor::InvalidateSelection(SelectionRange newMain, bool invalidateWholeSelection) {
  if (sel.Count() > 1 || !(sel.RangeMain().anchor == newMain.anchor) ||
      sel.IsRectangular()) {
    invalidateWholeSelection = true;
  }
  Sci::Position firstAffected =
      std::min(sel.RangeMain().Start().Position(), newMain.Start().Position());
  // +1 for lastAffected ensures caret repainted
  Sci::Position lastAffected =
      std::max(newMain.caret.Position() + 1, newMain.anchor.Position());
...
SelectionRange Editor::LineSelectionRange(SelectionPosition currentPos_,
                                          SelectionPosition anchor_) const {
  if (currentPos_ > anchor_) {
    anchor_ =
        SelectionPosition(pdoc->LineStart(pdoc->LineFromPosition(anchor_.Position())));
    currentPos_ =
        SelectionPosition(pdoc->LineEnd(pdoc->LineFromPosition(currentPos_.Position())));
  } else {
    currentPos_ =
        SelectionPosition(pdoc->LineStart(pdoc->LineFromPosition(currentPos_.Position())));
...
void Editor::MultipleSelectAdd(AddNumber addNumber) {
  if (SelectionEmpty() || !multipleSelection) {
    ...
    if (!pdoc->HasCaseFolder()) pdoc->SetCaseFolder(CaseFolderForEncoding());

    const Range rangeMainSelection(sel.RangeMain().Start().Position(),
                                   sel.RangeMain().End().Position());
    const std::string selectedText =
        RangeText(rangeMainSelection.start, rangeMainSelection.end);
    ...
    for (std::vector<Range>::const_iterator it = searchRanges.begin();
         it != searchRanges.end(); ++it) {
      ...
      for (;;) {
        ...
        const Sci::Position pos = pdoc->FindText(
            searchStart, searchEnd, selectedText.c_str(), searchFlags, &lengthFound);
        if (pos >= 0) {
          sel.AddSelection(SelectionRange(pos + lengthFound, pos));
          ContainerNeedsUpdate(SC_UPDATE_SELECTION);
          ScrollRange(sel.RangeMain());
          Redraw();
          if (addNumber == addOne) return;
...
bool Editor::RangeContainsProtected(Sci::Position start, Sci::Position end) const noexcept {
  if (vs.ProtectionActive()) {
    ...
    for (Sci::Position pos = start; pos < end; pos++) {
      if (vs.styles[pdoc->StyleIndexAt(pos)].IsProtected()) return true;
    }
  }
  return false;
}
 ...
 */
Sci::Position Editor::MovePositionOutsideChar(Sci::Position pos, Sci::Position moveDir,
                                              bool checkLineEnd) const {
  return MovePositionOutsideChar(SelectionPosition(pos), moveDir, checkLineEnd).Position();
}

SelectionPosition Editor::MovePositionOutsideChar(SelectionPosition pos,
                                                  Sci::Position moveDir,
                                                  bool checkLineEnd) const {
  const Sci::Position posMoved =
      pdoc->MovePositionOutsideChar(pos.Position(), moveDir, checkLineEnd);
  if (posMoved != pos.Position()) pos.SetPosition(posMoved);
  if (vs.ProtectionActive()) {
    if (moveDir > 0) {
      if ((pos.Position() > 0) &&
...
void Editor::MovePositionTo(SelectionPosition newPos, Selection::selTypes selt,
                            bool ensureVisible) {
...
void Editor::MovePositionTo(Sci::Position newPos, Selection::selTypes selt,
                            bool ensureVisible) {
...
SelectionPosition Editor::MovePositionSoVisible(SelectionPosition pos, int moveDir) {
  ...
  if (pcs->GetVisible(lineDoc)) {
    ...
    if (moveDir > 0) {
      ...
      return SelectionPosition(pdoc->LineStart(pcs->DocFromDisplay(lineDisplay)));
    } else {
      lineDisplay = std::clamp<Sci::Line>(lineDisplay - 1, 0, pcs->LinesDisplayed());
      return SelectionPosition(pdoc->LineEnd(pcs->DocFromDisplay(lineDisplay)));
    }
  }
}
...
void Editor::VerticalCentreCaret() {
  const Sci::Line lineDoc = pdoc->SciLineFromPosition(
      sel.IsRectangular() ? sel.Rectangular().caret.Position() : sel.MainCaret());
...
void Editor::MoveSelectedLines(int lineDelta) {
  ...
  if (selectionEnd > beginningOfEndLine || selectionStart == selectionEnd) {
    ...
    appendEol = (selectionEnd == pdoc->Length() &&
                 pdoc->SciLineFromPosition(selectionEnd) == endLine);
  }
  ...
  if ((selectionStart == 0 && lineDelta < 0) ||
      (selectionEnd == pdoc->Length() && lineDelta > 0) || selectionStart == selectionEnd) {
    return;
  }
  ...
  Sci::Position selectionLength =
      pdoc->InsertString(CurrentPosition(), selectedText.Data(), selectedText.Length());
  if (appendEol) {
    const Sci::Position lengthInserted =
        pdoc->InsertString(CurrentPosition() + selectionLength, eol, strlen(eol));
...
void Editor::MoveCaretInsideView(bool ensureVisible) {
  ...
  if (pt.y < rcClient.top) {
    MovePositionTo(SPositionFromLocation(Point::FromInts(lastXChosen - xOffset,
                                                         static_cast<int>(rcClient.top)),
                                         false, false, UserVirtualSpace()),
                   Selection::noSel, ensureVisible);
  } else if ((pt.y + vs.lineHeight - 1) > rcClient.bottom) {
    const ptrdiff_t yOfLastLineFullyDisplayed =
        static_cast<ptrdiff_t>(rcClient.top) + (LinesOnScreen() - 1) * vs.lineHeight;
    MovePositionTo(
        SPositionFromLocation(
            Point::FromInts(lastXChosen - xOffset,
                            static_cast<int>(rcClient.top + yOfLastLineFullyDisplayed)),
            false, false, UserVirtualSpace()),
        Selection::noSel, ensureVisible);
  }
}
...
'3UZ' notation is used to indicate three time the size of the UZ as a distance to the
margin.
...
where most code reside, and the lines after the caret, eg. the body of a function.
     ...
     |        |       |      |                                            |
slop | strict | jumps | even | Caret can go to the margin                 | When reaching
limit (caret going out of |        |       |      | | visibility or going into the UZ)
display is...
-----+--------+-------+------+--------------------------------------------+--------------------------------------------------------------
  0  |   0    |   0   |   0  | Yes                                        | moved to put
caret on top/on right 0  |   0    |   0   |   1  | Yes | moved by one position 0  |   0    |
1   |   0  | Yes                                        | moved to put caret on top/on right
  0  |   0    |   1   |   1  | Yes                                        | centred on the
caret 0  |   1    |   -   |   0  | Caret is always on top/on right of display | - 0  |   1
|   -   |   1  | No, caret is always centred                | - 1  |   0    |   0   |   0  |
Yes                                        | moved to put caret out of the asymmetrical UZ
  1  |   0    |   0   |   1  | Yes                                        | moved to put
caret out of the UZ 1  |   0    |   1   |   0  | Yes | moved to put caret at 3UZ of the top
or right margin 1  |   0    |   1   |   1  | Yes                                        |
moved to put caret at 3UZ of the margin 1  |   1    |   -   |   0  | Caret is always at UZ
of top/right margin  | - 1  |   1    |   0   |   1  | No, kept out of UZ | moved by one
position 1  |   1    |   1   |   1  | No, kept out of UZ                         | moved to
put caret at 3UZ of the margin
*/
...
Editor::XYScrollPosition Editor::XYScrollToMakeVisible(const SelectionRange &range,
  ...
  if ((options & xysVertical) &&
      (pt.y < rcClient.top || ptBottomCaret.y >= rcClient.bottom ||
       (policies.y.policy & CARET_STRICT) != 0)) {
  ...
  if ((options & xysHorizontal) && !Wrapping()) {
    ...
    if (bSlop) { // A margin is defined
      ...
      if (bStrict || (bJump && (pt.x < rcClient.left || pt.x >= rcClient.right))) {
    ...
    // In case of a jump (find result) largely out of display, adjust the offset to display
    // the caret
...
void Editor::EnsureCaretVisible(bool useMargin, bool vert, bool horiz) {
  SetXYScroll(XYScrollToMakeVisible(
      SelectionRange(posDrag.IsValid() ? posDrag : sel.RangeMain().caret),
      static_cast<XYScrollOptions>((useMargin ? xysUseMargin : 0) |
                                   (vert ? xysVertical : 0) | (horiz ? xysHorizontal : 0)),
      caretPolicies));
}
...
void Editor::ShowCaretAtCurrentPosition() {
  if (hasFocus) {
    ...
    if (caret.period > 0) FineTickerStart(tickCaret, caret.period, caret.period / 10);
  } else {
    caret.active = false;
    caret.on = false;
    FineTickerCancel(tickCaret);
...
bool Editor::WrapOneLine(Surface *surface, Sci::Line lineToWrap) {
  ...
  return pcs->SetHeight(lineToWrap,
                        linesWrapped +
  ...
  if (!Wrapping()) {
    if (wrapWidth != LineLayout::wrapWidthInfinite) {
      ...
      for (Sci::Line lineDoc = 0; lineDoc < pdoc->LinesTotal(); lineDoc++) {
        pcs->SetHeight(lineDoc,
                       1 + (vs.annotationVisible ? pdoc->AnnotationLines(lineDoc) : 0));
      }
      wrapOccurred = true;
    }
    wrapPending.Reset();
    ...
    if (ws == WrapScope::wsVisible) {
      ...
      while ((lineToWrapEnd < pcs->LinesInDoc()) && (lines > 0)) {
        if (pcs->GetVisible(lineToWrapEnd)) lines--;
        lineToWrapEnd++;
      }
    ...
    if (lineToWrap < lineToWrapEnd) {
      ...
      if (surface) {
        ...
        goodTopLine =
            pcs->DisplayFromDoc(lineDocTop) +
            std::min(subLineTop, static_cast<Sci::Line>(pcs->GetHeight(lineDocTop) - 1));
      }
    }
...
void Editor::LinesJoin() {
  if (!RangeContainsProtected(targetRange.start.Position(), targetRange.end.Position())) {
    UndoGroup ug(pdoc);
    bool prevNonWS = true;
    for (Sci::Position pos = targetRange.start.Position(); pos < targetRange.end.Position();
         pos++) {
...
void Editor::PaintSelMargin(Surface *surfaceWindow, const PRectangle &rc) {
  if (vs.fixedColumnWidth == 0) return;
  ...
  if (!rc.Intersects(rcMargin)) return;
  ...
  if (rcMargin.bottom > rc.bottom) rcMargin.bottom = rc.bottom;
  if (rcMargin.top < rc.top) rcMargin.top = rc.top;
...
void Editor::Paint(Surface *surfaceWindow, PRectangle rcArea) {
  ...
  RefreshStyleData();
  if (paintState == paintAbandoned) return; // Scroll bars may have changed so need redraw
  RefreshPixMaps(surfaceWindow);
  ...
  if (!view.bufferedDraw) surfaceWindow->SetClip(rcArea);
  ...
  if (horizontalScrollBarVisible && trackLineWidth &&
      (view.lineWidthMaxSeen > scrollWidth)) {
...
Sci::Position Editor::RealizeVirtualSpace(Sci::Position position,
  ...
  if (virtualSpace > 0) {
    ...
    if (indent == position) {
      ...
      const Sci::Position lengthInserted =
          pdoc->InsertString(position, spaceText.c_str(), virtualSpace);
...
SelectionPosition Editor::RealizeVirtualSpace(const SelectionPosition &position) {
  ...
  return SelectionPosition(
      RealizeVirtualSpace(position.Position(), position.VirtualSpace()));
}
  ...
  FilterSelections();
  {
    ...
    for (std::vector<SelectionRange *>::reverse_iterator rit = selPtrs.rbegin();
      ...
      if (!RangeContainsProtected(currentSel->Start().Position(),
        ...
        positionInsert =
            RealizeVirtualSpace(positionInsert, currentSel->caret.VirtualSpace());
        const Sci::Position lengthInserted =
            pdoc->InsertString(positionInsert, sv.data(), sv.length());
        if (lengthInserted > 0) {
...
void Editor::ClearSelection(bool retainMultipleSelections) {
  if (!sel.IsRectangular() && !retainMultipleSelections) FilterSelections();
  UndoGroup ug(pdoc);
  for (size_t r = 0; r < sel.Count(); r++) {
    if (!sel.Range(r).Empty()) {
      if (!RangeContainsProtected(sel.Range(r).Start().Position(),
...
void Editor::PasteRectangular(SelectionPosition pos, const char *ptr, Sci::Position len) {
  ...
  for (Sci::Position i = 0; i < len; i++) {
    if (IsEOLChar(ptr[i])) {
      if ((ptr[i] == '\r') || (!prevCr)) line++;
      if (line >= pdoc->LinesTotal()) {
        if (pdoc->eolMode != SC_EOL_LF) pdoc->InsertString(pdoc->Length(), "\r", 1);
        if (pdoc->eolMode != SC_EOL_CR) pdoc->InsertString(pdoc->Length(), "\n", 1);
      }
      ...
      if (!RangeContainsProtected(sel.Range(r).caret.Position(),
        ...
        if (sel.Range(r).Start().VirtualSpace()) {
          if (sel.Range(r).anchor < sel.Range(r).caret)
            sel.Range(r) = SelectionRange(RealizeVirtualSpace(
                sel.Range(r).anchor.Position(), sel.Range(r).anchor.VirtualSpace()));
          else
            sel.Range(r) = SelectionRange(RealizeVirtualSpace(
                sel.Range(r).caret.Position(), sel.Range(r).caret.VirtualSpace()));
        }
        if ((sel.Count() == 1) ||
...
void Editor::Undo() {
  if (pdoc->CanUndo()) {
    ...
    if (newPos >= 0) SetEmptySelection(newPos);
    EnsureCaretVisible();
  }
}
...
void Editor::DelCharBack(bool allowLineStartDeletion) {
  RefreshStyleData();
  if (!sel.IsRectangular()) FilterSelections();
  if (sel.IsRectangular()) allowLineStartDeletion = false;
  UndoGroup ug(pdoc, (sel.Count() > 1) || !sel.Empty());
  if (sel.Empty()) {
    for (size_t r = 0; r < sel.Count(); r++) {
      if (!RangeContainsProtected(sel.Range(r).caret.Position() - 1,
        ...
        if (sel.Range(r).caret.VirtualSpace()) {
          ...
          if (allowLineStartDeletion ||
              (pdoc->LineStart(lineCurrentPos) != sel.Range(r).caret.Position())) {
            if (pdoc->GetColumn(sel.Range(r).caret.Position()) <=
                    pdoc->GetLineIndentation(lineCurrentPos) &&
                pdoc->GetColumn(sel.Range(r).caret.Position()) > 0 &&
                pdoc->backspaceUnindents) {
              ...
              if (indentationChange == 0) indentationChange = indentationStep;
...
int Editor::ModifierFlags(bool shift, bool ctrl, bool alt, bool meta, bool super) noexcept {
  return (shift ? SCI_SHIFT : 0) | (ctrl ? SCI_CTRL : 0) | (alt ? SCI_ALT : 0) |
         (meta ? SCI_META : 0) | (super ? SCI_SUPER : 0);
}
...
bool Editor::NotifyMarginClick(Point pt, int modifiers) {
  ...
  if ((marginClicked >= 0) && vs.ms[marginClicked].sensitive) {
    ...
    if ((vs.ms[marginClicked].mask & SC_MASK_FOLDERS) &&
...
constexpr Sci::Position MovePositionForInsertion(Sci::Position position,
                                                 Sci::Position startInsertion,
                                                 Sci::Position length) noexcept {
...
constexpr Sci::Position MovePositionForDeletion(Sci::Position position,
...
void Editor::NotifyModified(Document *, DocModification mh, void *) {
  ...
  if (mh.modificationType & (SC_MOD_CHANGESTYLE | SC_MOD_CHANGEINDICATOR)) {
    ...
    if ((mh.modificationType & (SC_MOD_BEFOREINSERT | SC_MOD_BEFOREDELETE)) &&
      ...
      if (mh.modificationType & SC_MOD_BEFOREINSERT) {
        if (pdoc->ContainsLineEnd(mh.text, mh.length) &&
            (mh.position != pdoc->LineStart(lineOfPos)))
    ...
    if (mh.modificationType & SC_MOD_CHANGEANNOTATION) {
      ...
      if (vs.annotationVisible) {
        if (pcs->SetHeight(lineDoc, pcs->GetHeight(lineDoc) +
                                        static_cast<int>(mh.annotationLinesAdded))) {
          SetScrollBars();
        }
        Redraw();
      }
    ...
    if (mh.linesAdded != 0) {
      ...
      if (mh.position < posTopLine && !CanDeferToLastStep(mh)) {
        const Sci::Line newTop =
            std::clamp<Sci::Line>(topLine + mh.linesAdded, 0, MaxScrollPos());
        if (newTop != topLine) {
          SetTopLine(newTop);
          SetVerticalScrollPos();
        }
  ...
  if ((mh.modificationType & SC_MOD_CHANGEMARKER) ||
      (mh.modificationType & SC_MOD_CHANGEMARGIN)) {
  ...
  if ((mh.modificationType & SC_MOD_CHANGEFOLD) &&
      (foldAutomatic & SC_AUTOMATICFOLD_CHANGE)) {
...
void Editor::NotifyDeleted(Document *, void *) noexcept { /* Do nothing */ }
...
void Editor::PageMove(int direction, Selection::selTypes selt, bool stuttered) {
  ...
  if (stuttered && (direction < 0 && currentLine > topStutterLine)) {
    ...
    newPos = SPositionFromLocation(
        Point::FromInts(lastXChosen - xOffset, vs.lineHeight * caretPolicies.y.slop), false,
        false, UserVirtualSpace());
    ...
    newPos = SPositionFromLocation(
        Point::FromInts(lastXChosen - xOffset,
                        vs.lineHeight *
                            static_cast<int>(LinesToScroll() - caretPolicies.y.slop)),
        false, false, UserVirtualSpace());
    ...
    topLineNew =
        std::clamp<Sci::Line>(topLine + direction * LinesToScroll(), 0, MaxScrollPos());
    newPos = SPositionFromLocation(
        Point::FromInts(lastXChosen - xOffset,
                        static_cast<int>(pt.y) +
                            direction *
                                (vs.lineHeight * static_cast<int>(LinesToScroll()))),
        false, false, UserVirtualSpace());
  }
...
void Editor::ChangeCaseOfSelection(int caseMapping) {
  ...
  for (size_t r = 0; r < sel.Count(); r++) {
    ...
    if (rangeBytes > 0) {
      std::string sText =
          RangeText(currentNoVS.Start().Position(), currentNoVS.End().Position());
      ...
      if (sMapped != sText) {
        ...
        const Sci::Position lengthInserted =
            pdoc->InsertString(currentNoVS.Start().Position() + firstDifference,
                               sMapped.c_str() + firstDifference, lengthChange);
        // Automatic movement changes selection so reset to exactly the same as it was.
        const Sci::Position diffSizes =
            sMapped.size() - sText.size() + lengthInserted - lengthChange;
        if (diffSizes != 0) {
...
void Editor::LineTranspose() {
  ...
  if (line > 0) {
    ...
    startCurrent +=
        pdoc->InsertString(startPrevious, lineCurrent.c_str(), lineCurrent.length());
    pdoc->InsertString(startCurrent, linePrevious.c_str(), linePrevious.length());
...
void Editor::LineReverse() {
  const Sci::Line lineStart = pdoc->SciLineFromPosition(sel.RangeMain().Start().Position());
  const Sci::Line lineEnd = pdoc->SciLineFromPosition(sel.RangeMain().End().Position() - 1);
  ...
  sel.RangeMain() =
      SelectionRange(pdoc->LineStart(lineStart), pdoc->LineStart(lineEnd + 1));
}
...
SelectionPosition Editor::PositionUpOrDown(SelectionPosition spStart, int direction,
                                           int lastX) {
    ...
    if (direction < 0 && subLine == 0) {
      ...
      if (lineDisplay > 0) {
        skipLines = pdoc->AnnotationLines(pcs->DocFromDisplay(lineDisplay - 1));
               ...
               subLine >= (pcs->GetHeight(lineDoc) - 1 - pdoc->AnnotationLines(lineDoc))) {
      skipLines = pdoc->AnnotationLines(lineDoc);
    }
  }

  const Sci::Line newY =
      static_cast<Sci::Line>(pt.y) + (1 + skipLines) * direction * vs.lineHeight;
  if (lastX < 0) {
    lastX = static_cast<int>(pt.x) + xOffset;
  }
  SelectionPosition posNew =
      SPositionFromLocation(Point::FromInts(lastX - xOffset, static_cast<int>(newY)), false,
                            false, UserVirtualSpace());
...
void Editor::CursorUpOrDown(int direction, Selection::selTypes selt) {
  ...
  if (selt == Selection::selRectangle) {
    const SelectionRange rangeBase =
        sel.IsRectangular() ? sel.Rectangular() : sel.RangeMain();
    if (!sel.IsRectangular()) {
      InvalidateWholeSelection();
      sel.DropAdditionalRanges();
    }
    ...
    // Calculate new caret position and call SetSelection(), which will ensure whole lines
    // are selected.
    const SelectionPosition posNew =
        MovePositionSoVisible(PositionUpOrDown(caretToUse, direction, -1), direction);
    ...
    for (size_t r = 0; r < sel.Count(); r++) {
      ...
      const SelectionPosition posNew =
          MovePositionSoVisible(PositionUpOrDown(spCaretNow, direction, lastX), direction);
      sel.Range(r) = selt == Selection::selStream
                         ? SelectionRange(posNew, sel.Range(r).anchor)
...
int Editor::HorizontalMove(unsigned int iMessage) {
  ...
  if (IsRectExtend(iMessage, sel.IsRectangular() && sel.MoveExtends())) {
    ...
    SelectionPosition selAtLimit =
        (NaturalDirection(iMessage) > 0) ? sel.Limits().end : sel.Limits().start;
    switch (iMessage) {
    case SCI_HOME:
      selAtLimit =
          SelectionPosition(pdoc->LineStart(pdoc->LineFromPosition(selAtLimit.Position())));
      break;
    case SCI_VCHOME:
    ...
    for (size_t r = 0; r < sel.Count(); r++) {
      ...
      case SCI_VCHOME:
      case SCI_VCHOMEEXTEND:
        // VCHome alternates between beginning of line and beginning of text so may move
        // back or forwards
        spCaret = SelectionPosition(pdoc->VCHomePosition(spCaret.Position()));
        break;
      case SCI_VCHOMEDISPLAY:
      ...
      case SCI_CHARRIGHT:
        if (sel.Range(r).Empty()) {
          ...
          sel.Range(r) = SelectionRange((iMessage == SCI_CHARLEFT) ? sel.Range(r).Start()
                                                                   : sel.Range(r).End());
        }
        break;
...
int Editor::DelWordOrLine(unsigned int iMessage) {
  ...
  for (size_t r = 0; r < sel.Count(); r++) {
    if (leftwards) {
      ...
      sel.Range(r) = SelectionRange(RealizeVirtualSpace(sel.Range(r).caret));
    }

    Range rangeDelete;
    switch (iMessage) {
    case SCI_DELWORDLEFT:
      rangeDelete = Range(pdoc->NextWordStart(sel.Range(r).caret.Position(), -1),
                          sel.Range(r).caret.Position());
      break;
    case SCI_DELWORDRIGHT:
      rangeDelete = Range(sel.Range(r).caret.Position(),
    ...
    case SCI_DELLINELEFT:
      rangeDelete =
          Range(pdoc->LineStart(pdoc->LineFromPosition(sel.Range(r).caret.Position())),
                sel.Range(r).caret.Position());
      break;
    case SCI_DELLINERIGHT:
      rangeDelete =
          Range(sel.Range(r).caret.Position(),
...
int Editor::KeyCommand(unsigned int iMessage) {
  ...
  case SCI_LINECOPY: {
    ...
    CopyRangeToClipboard(pdoc->LineStart(lineStart), pdoc->LineStart(lineEnd + 1));
  } break;
  case SCI_LINECUT: {
    const Sci::Line lineStart = pdoc->SciLineFromPosition(SelectionStart().Position());
    const Sci::Line lineEnd = pdoc->SciLineFromPosition(SelectionEnd().Position());
  ...
  for (size_t r = 0; r < sel.Count(); r++) {
    ...
    if (lineOfAnchor == lineCurrentPos) {
      if (forwards) {
        ...
        if (pdoc->GetColumn(caretPosition) <=
          ...
          if (pdoc->useTabs) {
            ...
            int numSpaces = (pdoc->IndentSize()) -
                            (pdoc->GetColumn(caretPosition) % (pdoc->IndentSize()));
            if (numSpaces < 1) numSpaces = pdoc->IndentSize();
            const std::string spaceText(numSpaces, ' ');
            const Sci::Position lengthInserted =
                pdoc->InsertString(caretPosition, spaceText.c_str(), spaceText.length());
            sel.Range(r) = SelectionRange(caretPosition + lengthInserted);
          }
        }
      ...
      const Sci::Position anchorPosOnLine =
          sel.Range(r).anchor.Position() - pdoc->LineStart(lineOfAnchor);
      const Sci::Position currentPosPosOnLine =
          caretPosition - pdoc->LineStart(lineCurrentPos);
...
Sci::Position Editor::FindText(
    ...
    sptr_t lParam) { ///< @c Sci_TextToFind structure: The text to search for in the given
                     ///< range.
  ...
  if (!pdoc->HasCaseFolder()) pdoc->SetCaseFolder(CaseFolderForEncoding());
  try {
    const Sci::Position pos =
        pdoc->FindText(static_cast<Sci::Position>(ft->chrg.cpMin),
                       static_cast<Sci::Position>(ft->chrg.cpMax), ft->lpstrText,
                       static_cast<int>(wParam), &lengthFound);
    if (pos != -1) {
...
Sci::Position Editor::SearchText(
  ...
  try {
    if (iMessage == SCI_SEARCHNEXT) {
      pos = pdoc->FindText(searchAnchor, pdoc->Length(), txt, static_cast<int>(wParam),
                           &lengthFound);
    } else {
      pos = pdoc->FindText(searchAnchor, 0, txt, static_cast<int>(wParam), &lengthFound);
    }
  } catch (RegexError &) {
    errorStatus = SC_STATUS_WARN_REGEX;
...
Sci::Position Editor::SearchInTarget(const char *text, Sci::Position length) {
  ...
  try {
    const Sci::Position pos =
        pdoc->FindText(targetRange.start.Position(), targetRange.end.Position(), text,
...
void Editor::GoToLine(Sci::Line lineNo) {
  if (lineNo > pdoc->LinesTotal()) lineNo = pdoc->LinesTotal();
...
static bool Close(Point pt1, Point pt2, Point threshold) noexcept {
  ...
  if (std::abs(ptDifference.x) > threshold.x) return false;
  if (std::abs(ptDifference.y) > threshold.y) return false;
  return true;
}
...
void Editor::CopySelectionRange(SelectionText *ss, bool allowLineCopy) {
  if (sel.Empty()) {
    if (allowLineCopy) {
      ...
      if (pdoc->eolMode != SC_EOL_LF) text.push_back('\r');
      if (pdoc->eolMode != SC_EOL_CR) text.push_back('\n');
      ss->Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, false,
               true);
    }
  } else {
    std::string text;
    ...
    ss->Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet,
             sel.IsRectangular(), sel.selType == Selection::selLines);
  }
}
...
void Editor::CopyRangeToClipboard(Sci::Position start, Sci::Position end) {
  ...
  SelectionText selectedText;
  std::string text = RangeText(start, end);
  selectedText.Copy(text, pdoc->dbcsCodePage, vs.styles[STYLE_DEFAULT].characterSet, false,
                    false);
  CopyToClipboard(selectedText);
}
...
void Editor::CopyText(size_t length, const char *text) {
  SelectionText selectedText;
  selectedText.Copy(std::string(text, length), pdoc->dbcsCodePage,
                    vs.styles[STYLE_DEFAULT].characterSet, false, false);
  CopyToClipboard(selectedText);
}
...
void Editor::DropAt(SelectionPosition position, const char *value, size_t lengthValue,
                    bool moving, bool rectangular) {
  // Platform::DebugPrintf("DropAt %d %d\n", inDragDrop, position);
  if (inDragDrop == ddDragging) dropWentOutside = false;
  ...
  if ((inDragDrop != ddDragging) || !(positionWasInSelection) ||
        ...
        for (size_t r = 0; r < sel.Count(); r++) {
          if (position >= sel.Range(r).Start()) {
            if (position > sel.Range(r).End()) {
              ...
              positionAfterDeletion.Add(
                  -SelectionRange(position, sel.Range(r).Start()).Length());
            }
          }
        }
      } else {
    ...
    std::string convertedText =
        Document::TransformLineEnds(value, lengthValue, pdoc->eolMode);

    if (rectangular) {
      PasteRectangular(position, convertedText.c_str(), convertedText.length());
      // Should try to select new rectangle but it may not be a rectangle now so just select
      // the drop position
...
void Editor::DropAt(SelectionPosition position, const char *value, bool moving,
                    bool rectangular) {
...
bool Editor::PositionInSelection(Sci::Position pos) {
  ...
  for (size_t r = 0; r < sel.Count(); r++) {
    if (sel.Range(r).Contains(pos)) return true;
  }
  return false;
}
...
Window::Cursor Editor::GetMarginCursor(Point pt) const noexcept {
  ...
  for (const MarginStyle &m : vs.ms) {
    if ((pt.x >= x) && (pt.x < x + m.width)) return static_cast<Window::Cursor>(m.cursor);
    x += m.width;
  }
...
void Editor::LineSelection(Sci::Position lineCurrentPos_, Sci::Position lineAnchorPos_,
                           bool wholeLine) {
...
void Editor::WordSelection(Sci::Position pos) {
  if (pos < wordSelectAnchorStartPos) {
    ...
    // Skip ExtendWordSelect if the line is empty or if pos is the first position on the
    // line. This ensures that a series of empty lines isn't counted as a single "word".
...
static constexpr bool AllowVirtualSpace(int virtualSpaceOptions,
                                        bool rectangular) noexcept {
...
void Editor::ButtonDownWithModifiers(Point pt, unsigned int curTime, int modifiers) {
  ...
  // curTime - lastClickTime, alt, inDragDrop);
  ptMouseLast = pt;
  const bool ctrl = (modifiers & SCI_CTRL) != 0;
  const bool shift = (modifiers & SCI_SHIFT) != 0;
  const bool alt = (modifiers & SCI_ALT) != 0;
  SelectionPosition newPos =
      SPositionFromLocation(pt, false, false, AllowVirtualSpace(virtualSpaceOptions, alt));
  ...
  if (NotifyMarginClick(pt, modifiers)) return;
  ...
  if ((curTime < (lastClickTime + Platform::DoubleClickTime())) &&
      Close(pt, lastClick, doubleClickCloseThreshold)) {
    // Platform::DebugPrintf("Double click %d %d = %d\n", curTime, lastClickTime, curTime -
    // lastClickTime);
    SetMouseCapture(true);
    FineTickerStart(tickScroll, 100, 10);
    if (!ctrl || !multipleSelection ||
        (selectionType != selChar && selectionType != selWord))
    ...
    if (inSelMargin) {
      ...
      if (selectionType == selSubLine) {
        ...
        selectionType = (Wrapping() && (marginOptions & SC_MARGINOPTION_SUBLINESELECT))
                            ? selSubLine
                            : selWholeLine;
      }
    } else {
    ...
    if (selectionType == selWord) {
      ...
      if ((sel.MainCaret() >= originalAnchorPos) && !pdoc->IsLineEndPosition(charPos)) {
        startWord =
            pdoc->ExtendWordSelect(pdoc->MovePositionOutsideChar(charPos + 1, 1), -1);
    ...
    if (inSelMargin) {
      ...
      if (!shift) {
        ...
        // This ensures that we don't end up stuck in previous selection mode, which is no
        // longer valid. Otherwise, if there's a non empty selection, reset selection type
        // only if it differs from selSubLine and selWholeLine. This ensures that we
        // continue selecting in the same selection mode.
      ...
      if (inDragDrop != ddInitial) {
        ...
        if (!shift) {
          if (ctrl && multipleSelection) {
            ...
            InvalidateSelection(SelectionRange(newPos), true);
            if (sel.Count() > 1) Redraw();
            if ((sel.Count() > 1) || (sel.selType != Selection::selStream)) sel.Clear();
        ...
        if (shift)
          anchorCurrent =
              sel.IsRectangular() ? sel.Rectangular().anchor : sel.RangeMain().anchor;
        sel.selType = alt ? Selection::selRectangle : Selection::selStream;
        selectionType = selChar;
        originalAnchorPos = sel.MainCaret();
...
void Editor::RightButtonDownWithModifiers(Point pt, unsigned int, int modifiers) {
  if (NotifyMarginRightClick(pt, modifiers)) return;
}
...
void Editor::SetHoverIndicatorPosition(Sci::Position position) {
  ...
  if (!vs.indicatorsDynamic) return;
...
void Editor::ButtonMoveWithModifiers(Point pt, unsigned int, int modifiers) {
  ...
  SelectionPosition movePos = SPositionFromLocation(
      pt, false, false, AllowVirtualSpace(virtualSpaceOptions, sel.IsRectangular()));
  ...
  if (HaveMouseCapture()) {
    ...
    if (hotspot.Valid() && !PointIsHotspot(pt)) SetHotSpotRange(nullptr);

    if (hotSpotClickPos != INVALID_POSITION &&
        PositionFromLocation(pt, true, true) != hotSpotClickPos) {
...
void Editor::ButtonUpWithModifiers(Point pt, unsigned int curTime, int modifiers) {
  ...
  SelectionPosition newPos = SPositionFromLocation(
      pt, false, false, AllowVirtualSpace(virtualSpaceOptions, sel.IsRectangular()));
  ...
  if (HaveMouseCapture()) {
    ...
    if (inDragDrop == ddDragging) {
      ...
      if (selStart < selEnd) {
        if (drag.Length()) {
          ...
          if (modifiers & SCI_CTRL) {
            const Sci::Position lengthInserted =
                pdoc->InsertString(newPos.Position(), drag.Data(), length);
            if (lengthInserted > 0) {
      ...
      if (selectionType == selChar) {
        if (sel.Count() > 1) {
          sel.RangeMain() = SelectionRange(newPos, sel.Range(sel.Count() - 1).anchor);
          InvalidateWholeSelection();
        } else {
...
bool Editor::Idle() {
  ...
  const bool idleDone =
      !needWrap && !needIdleStyling; // && thatDone && theOtherThingDone...
...
Sci::Position Editor::PositionAfterArea(PRectangle rcArea) const {
  ...
  const Sci::Line lineAfter =
      TopLineOfMain() + static_cast<Sci::Line>(rcArea.bottom - 1) / vs.lineHeight + 1;
...
void Editor::StyleToPositionInView(Sci::Position pos) {
  ...
  if (pos > endWindow) pos = endWindow;
...
Sci::Position Editor::PositionAfterMaxStyling(Sci::Position posMax, bool scrolling) const {
  ...
  const Sci::Line linesToStyle =
      std::clamp(static_cast<int>(secondsAllowed / pdoc->durationStyleOneLine.Duration()),
                 10, 0x10000);
  const Sci::Line stylingMaxLine = std::min(
      pdoc->SciLineFromPosition(pdoc->GetEndStyled()) + linesToStyle, pdoc->LinesTotal());
...
void Editor::IdleStyling() {
  ...
  const Sci::Position endGoal =
      (idleStyling >= SC_IDLESTYLING_AFTERVISIBLE) ? pdoc->Length() : posAfterArea;
...
/**
 * Recursively expand a fold, making lines visible except where they have an unexpanded
 * parent.
 */
...
void Editor::EnsureLineVisible(Sci::Line lineDoc, bool enforcePolicy) {
  ...
  if (!pcs->GetVisible(lineDoc)) {
    ...
    if (lineParent >= 0) {
      if (lineDoc != lineParent) EnsureLineVisible(lineParent, enforcePolicy);
  ...
  if (enforcePolicy) {
    ...
    if (visiblePolicy.policy & VISIBLE_SLOP) {
      if ((topLine > lineDisplay) || ((visiblePolicy.policy & VISIBLE_STRICT) &&
                                      (topLine + visiblePolicy.slop > lineDisplay))) {
        SetTopLine(
            std::clamp<Sci::Line>(lineDisplay - visiblePolicy.slop, 0, MaxScrollPos()));
        SetVerticalScrollPos();
        Redraw();
      } else if ((lineDisplay > topLine + LinesOnScreen() - 1) ||
                 ((visiblePolicy.policy & VISIBLE_STRICT) &&
                  (lineDisplay > topLine + LinesOnScreen() - 1 - visiblePolicy.slop))) {
        SetTopLine(std::clamp<Sci::Line>(
            lineDisplay - LinesOnScreen() + 1 + visiblePolicy.slop, 0, MaxScrollPos()));
        SetVerticalScrollPos();
        Redraw();
      }
    } else {
      if ((topLine > lineDisplay) || (lineDisplay > topLine + LinesOnScreen() - 1) ||
          (visiblePolicy.policy & VISIBLE_STRICT)) {
        SetTopLine(std::clamp<Sci::Line>(lineDisplay - LinesOnScreen() / 2 + 1, 0,
                                         MaxScrollPos()));
        SetVerticalScrollPos();
        Redraw();
      }
...
void Editor::FoldAll(int action) {
  ...
  if (expanding) {
    ...
    for (Sci::Line line = 0; line < maxLine; line++) {
      const int level = pdoc->GetLevel(line);
      if ((level & SC_FOLDLEVELHEADERFLAG) && (SC_FOLDLEVELBASE == LevelNumber(level))) {
...
void Editor::FoldChanged(Sci::Line line, int levelNow, int levelPrev) {
  ...
  if (!(levelNow & SC_FOLDLEVELWHITEFLAG) &&
    ...
    if (pcs->HiddenLines()) {
      ...
      if ((parentLine < 0) ||
          (pcs->GetExpanded(parentLine) && pcs->GetVisible(parentLine))) {
        pcs->SetVisible(line, line, true);
        SetScrollBars();
        Redraw();
      }
  ...
  // Combining two blocks where the first one is collapsed (e.g. by adding characters in the
  // line which separates the two blocks)
  if (!(levelNow & SC_FOLDLEVELWHITEFLAG) &&
      (LevelNumber(levelPrev) < LevelNumber(levelNow))) {
    if (pcs->HiddenLines()) {
      const Sci::Line parentLine = pdoc->GetFoldParent(line);
...
Sci::Position Editor::ReplaceTarget(bool replacePatterns, const char *text,
                                    Sci::Position length) {
  UndoGroup ug(pdoc);
  ...
  Sci::Position startAfterSpaceInsertion =
      RealizeVirtualSpace(targetRange.start.Position(), targetRange.start.VirtualSpace());
  targetRange.start.SetPosition(startAfterSpaceInsertion);
  targetRange.end = targetRange.start;

  // Insert the new text
  const Sci::Position lengthInserted =
      pdoc->InsertString(targetRange.start.Position(), text, length);
...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
  ...
  if (recordingMacro) NotifyMacroRecord(iMessage, wParam, lParam);

  switch (iMessage) {

  case SCI_GETTEXT: {
    if (lParam == 0) return pdoc->Length() + 1;
  ...
  case SCI_GETLINE: { // Risk of overwriting the end of the buffer
    const Sci::Position lineStart = pdoc->LineStart(static_cast<Sci::Line>(wParam));
    const Sci::Position lineEnd = pdoc->LineStart(static_cast<Sci::Line>(wParam + 1));
    // not NUL terminated
  ...
  case SCI_SETSEL: {
    ...
    if (nEnd < 0) nEnd = pdoc->Length();
    if (nStart < 0) nStart = nEnd; // Remove selection
  ...
  case SCI_LINEFROMPOSITION:
    if (static_cast<Sci::Position>(wParam) < 0) return 0;
  ...
  case SCI_POSITIONFROMLINE:
    ...
    if (static_cast<Sci::Line>(wParam) > pdoc->LinesTotal()) return -1;
  ...
  case SCI_LINELENGTH:
    if ((static_cast<Sci::Position>(wParam) < 0) ||
      ...
      return 0;
    return pdoc->LineStart(static_cast<Sci::Position>(wParam) + 1) -
           pdoc->LineStart(static_cast<Sci::Position>(wParam));
  ...
  case SCI_REPLACESEL: {
    ...
    const Sci::Position lengthInserted =
        pdoc->InsertString(sel.MainCaret(), replacement, strlen(replacement));
    SetEmptySelection(sel.MainCaret() + lengthInserted);
    SetLastXChosen();
    EnsureCaretVisible();
  } break;
  ...
  case SCI_REPLACETARGET:
    ...
    return ReplaceTarget(false, CharPtrFromSPtr(lParam),
  ...
  case SCI_POSITIONRELATIVE:
    return std::clamp<Sci::Position>(
        pdoc->GetRelativePosition(static_cast<Sci::Position>(wParam), lParam), 0,
        pdoc->Length());

  case SCI_POSITIONRELATIVECODEUNITS:
    return std::clamp<Sci::Position>(
        pdoc->GetRelativePositionUTF16(static_cast<Sci::Position>(wParam), lParam), 0,
        pdoc->Length());

  case SCI_LINESCROLL:
    ScrollTo(topLine + static_cast<Sci::Line>(lParam));
    HorizontalScrollTo(xOffset +
                       static_cast<int>(wParam) * static_cast<int>(vs.spaceWidth));
    return 1;
  ...
  case SCI_GETTEXTRANGE: {
    ...
    if (cpMax == -1) cpMax = pdoc->Length();
    PLATFORM_ASSERT(cpMax <= pdoc->Length());
    Sci::Position len =
        cpMax -
        tr->chrg
            .cpMin; // No -1 as cpMin and cpMax are referring to inter character positions
  ...
  case SCI_ADDSTYLEDTEXT:
    if (lParam) AddStyledText(CharPtrFromSPtr(lParam), static_cast<Sci::Position>(wParam));
    return 0;

  case SCI_INSERTTEXT: {
    if (lParam == 0) return 0;
    Sci::Position insertPos = static_cast<Sci::Position>(wParam);
    if (insertPos == -1) insertPos = CurrentPosition();
    ...
    if (newCurrent > insertPos) newCurrent += lengthInserted;
  ...
  case SCI_APPENDTEXT:
    pdoc->InsertString(pdoc->Length(), CharPtrFromSPtr(lParam),
                       static_cast<Sci::Position>(wParam));
    return 0;
  ...
  case SCI_SETSELECTIONEND:
    SetSelection(static_cast<Sci::Position>(wParam),
                 std::min(sel.MainAnchor(), static_cast<Sci::Position>(wParam)));
    break;
  ...
  case SCI_MARKERHANDLEFROMLINE:
    return pdoc->MarkerHandleFromLine(static_cast<Sci::Line>(wParam),
                                      static_cast<int>(lParam));

  case SCI_MARKERNUMBERFROMLINE:
    return pdoc->MarkerNumberFromLine(static_cast<Sci::Line>(wParam),
                                      static_cast<int>(lParam));
  ...
  case SCI_POSITIONFROMPOINT:
    return PositionFromLocation(
        Point::FromInts(static_cast<int>(wParam) - vs.ExternalMarginWidth(),
                        static_cast<int>(lParam)),
        false, false);
  ...
  case SCI_SETTWOPHASEDRAW:
    if (view.SetTwoPhaseDraw(wParam != 0)) InvalidateStyleRedraw();
    break;
#endif
  ...
  case SCI_SETPHASESDRAW:
    if (view.SetPhasesDraw(static_cast<int>(wParam))) InvalidateStyleRedraw();
    break;
  ...
  case SCI_SETTABWIDTH:
    if (wParam > 0) {
      ...
      if (pdoc->indentInChars == 0) pdoc->actualIndentInChars = pdoc->tabInChars;
    }
    InvalidateStyleRedraw();
    break;
  ...
  case SCI_CLEARTABSTOPS:
    if (view.ClearTabstops(static_cast<Sci::Line>(wParam))) {
      const DocModification mh(SC_MOD_CHANGETABSTOPS, 0, 0, 0, nullptr,
                               static_cast<Sci::Line>(wParam));
  ...
  case SCI_SETVSCROLLBAR:
    if (verticalScrollBarVisible != (wParam != 0)) {
      ...
      SetScrollBars();
      ReconfigureScrollBars();
      if (verticalScrollBarVisible) SetVerticalScrollPos();
    }
    break;
  ...
  case SCI_SETBIDIRECTIONAL:
    // SCI_SETBIDIRECTIONAL is implemented on platform subclasses if they support
    // bidirectional text.
    break;
  ...
  case SCI_LINEFROMINDEXPOSITION:
    return pdoc->LineFromPositionIndex(static_cast<Sci::Position>(wParam),
                                       static_cast<int>(lParam));
  ...
  case SCI_MARKERSETALPHA:
    if (wParam <= MARKER_MAX) vs.markers[wParam].alpha = static_cast<int>(lParam);
    InvalidateStyleRedraw();
    break;
  case SCI_MARKERADD: {
    const int markerID =
        pdoc->AddMark(static_cast<Sci::Line>(wParam), static_cast<int>(lParam));
    return markerID;
  }
  ...
  case SCI_MARKERPREVIOUS: {
    for (Sci::Line iLine = static_cast<Sci::Line>(wParam); iLine >= 0; iLine--) {
      if ((pdoc->GetMark(iLine) & lParam) != 0) return iLine;
    }
  }
    return -1;
  ...
  case SCI_SETMARGINCURSORN:
    if (ValidMargin(wParam)) vs.ms[wParam].cursor = static_cast<int>(lParam);
    break;
  ...
  case SCI_SETMARGINS:
    if (wParam < 1000) vs.ms.resize(wParam);
    break;
  ...
  case SCI_SETFOLDLEVEL: {
    const int prev =
        pdoc->SetLevel(static_cast<Sci::Line>(wParam), static_cast<int>(lParam));
    if (prev != static_cast<int>(lParam)) RedrawSelMargin();
    return prev;
  }
  ...
  case SCI_HIDELINES:
    if (wParam > 0)
      pcs->SetVisible(static_cast<Sci::Line>(wParam), static_cast<Sci::Line>(lParam),
                      false);
    SetScrollBars();
    Redraw();
    break;
  ...
  case SCI_FOLDCHILDREN:
    FoldExpand(static_cast<Sci::Line>(wParam), static_cast<int>(lParam),
  ...
  case SCI_EXPANDCHILDREN:
    FoldExpand(static_cast<Sci::Line>(wParam), SC_FOLDACTION_EXPAND,
               static_cast<int>(lParam));
    break;
  ...
  case SCI_ASSIGNCMDKEY:
    kmap.AssignCmdKey(LowShortFromWParam(wParam), HighShortFromWParam(wParam),
  ...
  case SCI_INDICATORCLEARRANGE:
    pdoc->DecorationFillRange(static_cast<Sci::Position>(wParam), 0, lParam);
    break;
  ...
  case SCI_MULTIEDGECLEARALL:
    std::vector<EdgeProperties>().swap(
        vs.theMultiEdge); // Free vector and memory, C++03 compatible
    InvalidateStyleRedraw();
    break;
  ...
  case SCI_SETSELECTIONMODE: {
    ...
    case SC_SEL_LINES:
      ...
      SetSelection(sel.RangeMain().caret,
                   sel.RangeMain().anchor); // adjust current selection
      break;
    case SC_SEL_THIN:
  ...
  case SCI_GETLINESELENDPOSITION: {
    ...
    for (size_t r = 0; r < sel.Count(); r++) {
      ...
      if (portion.start.IsValid()) {
        return (iMessage == SCI_GETLINESELSTARTPOSITION) ? portion.start.Position()
  ...
  case SCI_GETREPRESENTATION: {
    const Representation *repr =
        reprs.RepresentationFromCharacter(ConstCharPtrFromUPtr(wParam), UTF8MaxBytes);
    if (repr) {
  ...
  case SCI_GETRANGEPOINTER:
    return reinterpret_cast<sptr_t>(
        pdoc->RangePointer(static_cast<Sci::Position>(wParam), lParam));
  ...
  case SCI_ANNOTATIONSETSTYLES:
    pdoc->AnnotationSetStyles(static_cast<Sci::Line>(wParam),
                              ConstUCharPtrFromSPtr(lParam));
    break;
  ...
  case SCI_SETRECTANGULARSELECTIONCARET:
    if (!sel.IsRectangular()) sel.Clear();
    sel.selType = Selection::selRectangle;
    sel.Rectangular().caret.SetPosition(static_cast<Sci::Position>(wParam));
    SetRectangularRange();
    Redraw();
