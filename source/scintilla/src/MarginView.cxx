...
void MarginView::DropGraphics(bool freeObjects) {
  if (freeObjects) {
    ...
    if (pixmapSelMargin) pixmapSelMargin->Release();
    if (pixmapSelPattern) pixmapSelPattern->Release();
    if (pixmapSelPatternOffset1) pixmapSelPatternOffset1->Release();
  }
}

void MarginView::AllocateGraphics(const ViewStyle &vsDraw) {
  if (!pixmapSelMargin) pixmapSelMargin.reset(Surface::Allocate(vsDraw.technology));
  if (!pixmapSelPattern) pixmapSelPattern.reset(Surface::Allocate(vsDraw.technology));
  if (!pixmapSelPatternOffset1)
    pixmapSelPatternOffset1.reset(Surface::Allocate(vsDraw.technology));
}

void MarginView::RefreshPixMaps(Surface *surfaceWindow, WindowID wid,
  ...
  if (!pixmapSelPattern->Initialised()) {
    ...
    // This complex procedure is to reproduce the checkerboard dithered pattern used by
    // windows for scroll bars and Visual Studio for its selection margin. The colour of
    // this pattern is half way between the chrome colour and the chrome highlight colour
    // making a nice transition between the window chrome and the content area. And it works
    // in low colour depths.
    const PRectangle rcPattern = PRectangle::FromInts(0, 0, patternSize, patternSize);

    // Initialize default colours based on the chrome colour scheme.  Typically the
    // highlight is white.
    ...
    if (!(vsDraw.selbarlight == ColourDesired(0xff, 0xff, 0xff))) {
      ...
      // colour. (Typically, the highlight colour is white.)
...
static int SubstituteMarkerIfEmpty(int markerCheck, int markerDefault,
                                   const ViewStyle &vs) {
  if (vs.markers[markerCheck].markType == SC_MARK_EMPTY) return markerDefault;
  return markerCheck;
}

void MarginView::PaintMargin(Surface *surface, Sci::Line topLine, PRectangle rc,
                             PRectangle rcMargin, const EditModel &model,
                             const ViewStyle &vs) {

  PRectangle rcSelMargin = rcMargin;
  rcSelMargin.right = rcMargin.left;
  if (rcSelMargin.bottom < rc.bottom) rcSelMargin.bottom = rc.bottom;
    ...
    if (vs.ms[margin].width > 0) {
      ...
      if (vs.ms[margin].style != SC_MARGIN_NUMBER) {
        if (vs.ms[margin].mask & SC_MASK_FOLDERS) {
          ...
          surface->FillRectangle(rcSelMargin, invertPhase ? *pixmapSelPattern
      ...
      const int lineStartPaint =
          static_cast<int>(rcMargin.top + ptOrigin.y) / vs.lineHeight;
      Sci::Line visibleLine = model.TopLineOfMain() + lineStartPaint;
      Sci::Position yposScreen =
          lineStartPaint * vs.lineHeight - static_cast<Sci::Position>(ptOrigin.y);
      ...
      if (vs.ms[margin].mask & SC_MASK_FOLDERS) {
        ...
        if (level & SC_FOLDLEVELWHITEFLAG) {
          ...
          if (!(levelPrev & SC_FOLDLEVELHEADERFLAG)) {
            if (LevelNumber(level) < LevelNumber(levelPrev)) needWhiteClosure = true;
          }
        }
        if (highlightDelimiter.isEnabled) {
          const Sci::Line lastLine =
              model.pcs->DocFromDisplay(topLine + model.LinesOnScreen()) + 1;
          model.pdoc->GetHighlightDelimiters(
              highlightDelimiter, model.pdoc->SciLineFromPosition(model.sel.MainCaret()),
              lastLine);
        }
      }

      // Old code does not know about new markers needed to distinguish all cases
      const int folderOpenMid =
          SubstituteMarkerIfEmpty(SC_MARKNUM_FOLDEROPENMID, SC_MARKNUM_FOLDEROPEN, vs);
      const int folderEnd =
          SubstituteMarkerIfEmpty(SC_MARKNUM_FOLDEREND, SC_MARKNUM_FOLDER, vs);
      ...
      while ((visibleLine < model.pcs->LinesDisplayed()) && yposScreen < rc.bottom) {
          ...
          if (firstSubLine) {
            ...
            if (model.foldFlags & (SC_FOLDFLAG_LEVELNUMBERS | SC_FOLDFLAG_LINESTATE)) {
              ...
              if (model.foldFlags & SC_FOLDFLAG_LEVELNUMBERS) {
                ...
                sprintf(
                    number, "%c%c %03X %03X", (lev & SC_FOLDLEVELHEADERFLAG) ? 'H' : '_',
            ...
            rcWrapMarker.left =
                rcWrapMarker.right - vs.styles[STYLE_LINENUMBER].aveCharWidth;
        ...
        } else if (vs.ms[margin].style == SC_MARGIN_TEXT ||
                   vs.ms[margin].style == SC_MARGIN_RTEXT) {
          const StyledText stMargin = model.pdoc->MarginStyledText(lineDoc);
          if (stMargin.text && ValidStyledText(vs, vs.marginStyleOffset, stMargin)) {
            if (firstSubLine) {
              surface->FillRectangle(
                  rcMarker, vs.styles[stMargin.StyleAt(0) + vs.marginStyleOffset].back);
              if (vs.ms[margin].style == SC_MARGIN_RTEXT) {
                const int width =
                    WidestLineWidth(surface, vs, vs.marginStyleOffset, stMargin);
                rcMarker.left = rcMarker.right - width - 3;
              }
              DrawStyledText(surface, vs, vs.marginStyleOffset, rcMarker, stMargin, 0,
                             stMargin.length, drawAll);
            } else {
              // if we're displaying annotation lines, colour the margin to match the
        ...
        if (marks) {
          for (int markBit = 0; (markBit < 32) && marks; markBit++) {
            if (marks & 1) {
              ...
              vs.markers[markBit].Draw(surface, rcMarker, fontLineNumber, tFold,
                                       vs.ms[margin].style);
            }
            marks >>= 1;
          }
