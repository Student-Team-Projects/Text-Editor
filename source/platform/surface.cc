...
Surface *Surface::Allocate(int technology) { return new TScintillaSurface; }
...
bool TScintillaSurface::Initialised() { return surface; }

void TScintillaSurface::PenColour(ColourDesired fore) {}

int TScintillaSurface::LogPixelsY() { return 1; }

int TScintillaSurface::DeviceHeightFont(int points) { return 1; }

void TScintillaSurface::MoveTo(int x_, int y_) {}

void TScintillaSurface::LineTo(int x_, int y_) {}

void TScintillaSurface::Polygon(Point *pts, size_t npts, ColourDesired fore,
                                ColourDesired back) {}

void TScintillaSurface::RectangleDraw(PRectangle rc, ColourDesired fore,
                                      ColourDesired back) {}

void TScintillaSurface::FillRectangle(PRectangle rc, ColourDesired back) {
  auto r = clipRect(rc);
  if (surface && 0 <= r.a.x && r.a.x < r.b.x && 0 <= r.a.y && r.a.y < r.b.y) {
...
void TScintillaSurface::RoundedRectangle(PRectangle rc, ColourDesired fore,
                                         ColourDesired back) {}

void TScintillaSurface::AlphaRectangle(PRectangle rc, int cornerSize, ColourDesired fill,
                                       int alphaFill, ColourDesired outline,
                                       int alphaOutline, int flags) {
  auto r = clipRect(rc);
  if (surface && 0 <= r.a.x && r.a.x < r.b.x && 0 <= r.a.y && r.a.y < r.b.y) {
    auto fg = convertColor(ColourDesired(alphaOutline)), bg = convertColor(fill);
    auto *cells = &surface->at(r.a.y, r.a.x);
    size_t count = r.b.x - r.a.x;
    for (int y = r.a.y; y < r.b.y; ++y) {
      for (size_t x = 0; x < count; ++x) {
        if (!fg.isDefault()) ::setFore(cells[x].attr, fg);
        if (!bg.isDefault()) ::setBack(cells[x].attr, bg);
      }
      cells += surface->size.x;
    }
  }
...
void TScintillaSurface::GradientRectangle(PRectangle rc,
                                          const std::vector<ColourStop> &stops,
                                          GradientOptions options) {}

void TScintillaSurface::DrawRGBAImage(PRectangle rc, int width, int height,
...
TScintillaSurface::Layout(const IScreenLine *screenLine) {
  return nullptr;
}

void TScintillaSurface::DrawTextNoClip(PRectangle rc, Font &font_, XYPOSITION ybase,
...
void TScintillaSurface::DrawTextClipped(PRectangle rc, Font &font_, XYPOSITION ybase,
                                        std::string_view text, ColourDesired fore,
                                        ColourDesired back) {
  auto r = clipRect(rc);
  if (surface && 0 <= r.a.x && r.a.x < r.b.x && 0 <= r.a.y && r.a.y < r.b.y) {
...
void TScintillaSurface::DrawTextTransparent(PRectangle rc, Font &font_, XYPOSITION ybase,
                                            std::string_view text, ColourDesired fore) {
  auto r = clipRect(rc);
  if (surface && 0 <= r.a.x && r.a.x < r.b.x && 0 <= r.a.y && r.a.y < r.b.y) {
...
void TScintillaSurface::MeasureWidths(Font &font_, std::string_view text,
...
XYPOSITION TScintillaSurface::Ascent(Font &font_) { return 0; }

XYPOSITION TScintillaSurface::Descent(Font &font_) { return 0; }

XYPOSITION TScintillaSurface::InternalLeading(Font &font_) { return 0; }

XYPOSITION TScintillaSurface::Height(Font &font_) { return 1; }

XYPOSITION TScintillaSurface::AverageCharWidth(Font &font_) { return 1; }

void TScintillaSurface::SetClip(PRectangle rc) {
  clip = rc;
  if (surface) clip.intersect({0, 0, surface->size.x, surface->size.y});
}
