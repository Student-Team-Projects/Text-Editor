...
GtkWidget *PWidget(WindowID wid) noexcept { return static_cast<GtkWidget *>(wid); }
...
class FontHandle : public Font {
  ...
  explicit FontHandle(const FontParameters &fp)
    ...
    if (fd) {
      pango_font_description_set_family(fd.get(), (fp.faceName[0] == '!') ? fp.faceName + 1
                                                                          : fp.faceName);
      ...
      pango_font_description_set_style(fd.get(),
                                       fp.italic ? PANGO_STYLE_ITALIC : PANGO_STYLE_NORMAL);
      pango_font_description_set_stretch(
          fd.get(), static_cast<PangoStretch>(static_cast<int>(fp.stretch) - 1));
    }
  }
...
class SurfaceImpl : public Surface {
  ...
  SurfaceImpl(cairo_t *context_, int width, int height, SurfaceMode mode_,
  ...
  void MeasureWidths(const Font *font_, std::string_view text,
                     XYPOSITION *positions) override;
  XYPOSITION WidthText(const Font *font_, std::string_view text) override;

  void DrawTextBaseUTF8(PRectangle rc, const Font *font_, XYPOSITION ybase,
                        std::string_view text, ColourRGBA fore);
...
const Supports SupportsGTK[] = {
    Supports::LineDrawsFinal,          Supports::FractionalStrokeWidth,
    Supports::TranslucentStroke,       Supports::PixelModification,
...
void SurfaceImpl::PenColourAlpha(ColourRGBA fore) noexcept {
  if (context) {
    cairo_set_source_rgba(context, fore.GetRedComponent(), fore.GetGreenComponent(),
                          fore.GetBlueComponent(), fore.GetAlphaComponent());
  }
}
...
SurfaceImpl::SurfaceImpl(cairo_t *context_, int width, int height, SurfaceMode mode_,
                         WindowID wid) noexcept {
  if (height > 0 && width > 0) {
    cairo_surface_t *psurfContext = cairo_get_target(context_);
    surf.reset(cairo_surface_create_similar(psurfContext, CAIRO_CONTENT_COLOR_ALPHA, width,
                                            height));
...
void SurfaceImpl::RoundedRectangle(PRectangle rc, FillStroke fillStroke) {
  if (((rc.right - rc.left) > 4) && ((rc.bottom - rc.top) > 4)) {
    ...
    Point pts[] = {
        Point(rc.left + 2, rc.top),     Point(rc.right - 2, rc.top),
        Point(rc.right, rc.top + 2),    Point(rc.right, rc.bottom - 2),
        Point(rc.right - 2, rc.bottom), Point(rc.left + 2, rc.bottom),
        Point(rc.left, rc.bottom - 2),  Point(rc.left, rc.top + 2),
    };
...
static void PathRoundRectangle(cairo_t *context, double left, double top, double width,
                               double height, double radius) noexcept {
  cairo_new_sub_path(context);
  cairo_arc(context, left + width - radius, top + radius, radius, -90 * degrees,
            0 * degrees);
  cairo_arc(context, left + width - radius, top + height - radius, radius, 0 * degrees,
            90 * degrees);
  cairo_arc(context, left + radius, top + height - radius, radius, 90 * degrees,
            180 * degrees);
...
void SurfaceImpl::AlphaRectangle(PRectangle rc, XYPOSITION cornerSize,
  ...
  if (context && rc.Width() > 0) {
    ...
    PenColourAlpha(fillStroke.fill.colour);
    if (cornerSize > 0)
      PathRoundRectangle(context, rc.left + fillStroke.stroke.width,
                         rc.top + fillStroke.stroke.width, rc.Width() - doubleStroke,
                         rc.Height() - doubleStroke, cornerSize);
    else
      cairo_rectangle(context, rc.left + fillStroke.stroke.width,
                      rc.top + fillStroke.stroke.width, rc.Width() - doubleStroke,
                      rc.Height() - doubleStroke);
    cairo_fill(context);

    PenColourAlpha(fillStroke.stroke.colour);
    if (cornerSize > 0)
      PathRoundRectangle(context, rc.left + halfStroke, rc.top + halfStroke,
                         rc.Width() - fillStroke.stroke.width,
                         rc.Height() - fillStroke.stroke.width, cornerSize);
    else
      cairo_rectangle(context, rc.left + halfStroke, rc.top + halfStroke,
                      rc.Width() - fillStroke.stroke.width,
                      rc.Height() - fillStroke.stroke.width);
    cairo_set_line_width(context, fillStroke.stroke.width);
    cairo_stroke(context);
  }
}
  ...
  if (context) {
    ...
    for (const ColourStop &stop : stops) {
      cairo_pattern_add_color_stop_rgba(
          pattern, stop.position, stop.colour.GetRedComponent(),
          stop.colour.GetGreenComponent(), stop.colour.GetBlueComponent(),
...
void SurfaceImpl::DrawRGBAImage(PRectangle rc, int width, int height,
  ...
  PLATFORM_ASSERT(context);
  if (width == 0) return;
  if (rc.Width() > width) rc.left += (rc.Width() - width) / 2;
  rc.right = rc.left + width;
  if (rc.Height() > height) rc.top += (rc.Height() - height) / 2;
  ...
  UniqueCairoSurface surfImage(cairo_image_surface_create_for_data(
      &image[0], CAIRO_FORMAT_ARGB32, width, height, stride));
...
void SurfaceImpl::Stadium(PRectangle rc, FillStroke fillStroke, Ends ends) {
  ...
  default:
    cairo_move_to(context, rcInner.left + halfStroke, rc.top + halfStroke);
    cairo_arc_negative(context, rcInner.left + halfStroke, midLine, radius, 270 * degrees,
                       90 * degrees);
    break;
  }
  ...
  default:
    cairo_line_to(context, rcInner.right - halfStroke, rc.bottom - halfStroke);
    cairo_arc_negative(context, rcInner.right - halfStroke, midLine, radius, 90 * degrees,
                       270 * degrees);
    break;
  }
...
void SurfaceImpl::Copy(PRectangle rc, Point from, Surface &surfaceSource) {
  ...
  if (canDraw) {
    PLATFORM_ASSERT(context);
    cairo_set_source_surface(context, surfi.surf.get(), rc.left - from.x, rc.top - from.y);
    cairo_rectangle(context, rc.left, rc.top, rc.Width(), rc.Height());
    cairo_fill(context);
  }
}

std::unique_ptr<IScreenLineLayout> SurfaceImpl::Layout(const IScreenLine *) { return {}; }
...
void SurfaceImpl::DrawTextBase(PRectangle rc, const Font *font_, XYPOSITION ybase,
                               std::string_view text, ColourRGBA fore) {
  if (context) {
    PenColourAlpha(fore);
  ...
  if (PFont(font_)->fd) {
    ...
    if (et == EncodingType::utf8) {
      ...
      std::string utfForm;
      {
        ...
        UniqueStr textInUTF8(g_convert(text.data(), text.length(), "UTF-8", charSetID,
      ...
      if (et == EncodingType::dbcs) {
        if (!utfForm.empty()) {
          ...
          while (!iti.finished) {
            ...
            const int places =
                g_utf8_strlen(utfForm.data() + clusterStart, clusterEnd - clusterStart);
            int place = 1;
            while (clusterStart < clusterEnd) {
              size_t lenChar =
                  MultiByteLenFromIconv(convMeasure, text.data() + i, text.length() - i);
              while (lenChar--) {
                positions[i++] = iti.position - (places - place) * iti.distance / places;
                positionsCalculated++;
              }
              clusterStart +=
                  UTF8BytesOfLead[static_cast<unsigned char>(utfForm[clusterStart])];
              place++;
            }
          }
          PLATFORM_ASSERT(static_cast<size_t>(i) == text.length());
      ...
      if (positionsCalculated < 1) {
        ...
        while (!iti.finished) {
          iti.Next();
          const int clusterEnd = iti.curIndex;
          const int ligatureLength =
              g_utf8_strlen(utfForm.data() + clusterStart, clusterEnd - clusterStart);
          if (((i + ligatureLength) > lenPositions) ||
              (rtlCheck && ((clusterEnd <= clusterStart) || (ligatureLength == 0) ||
                            (ligatureLength > 3)))) {
// Something has gone wrong: exit quickly but pretend all the characters are equally spaced:
          ...
          for (int charInLig = 0; charInLig < ligatureLength; charInLig++) {
            positions[i++] = iti.position - (ligatureLength - 1 - charInLig) *
                                                iti.distance / ligatureLength;
          }
...
void SurfaceImpl::DrawTextBaseUTF8(PRectangle rc, const Font *font_, XYPOSITION ybase,
                                   std::string_view text, ColourRGBA fore) {
  if (context) {
    PenColourAlpha(fore);
    const XYPOSITION xText = rc.left;
...
XYPOSITION SurfaceImpl::Ascent(const Font *font_) {
  ...
  UniquePangoFontMetrics metrics(
      pango_context_get_metrics(pcontext.get(), PFont(font_)->fd.get(), language));
  return std::max(
      1.0, std::ceil(pango_units_to_double(pango_font_metrics_get_ascent(metrics.get()))));
}
...
XYPOSITION SurfaceImpl::InternalLeading(const Font *) { return 0; }

XYPOSITION SurfaceImpl::Height(const Font *font_) { return Ascent(font_) + Descent(font_); }
...
GdkRectangle MonitorRectangleForWidget(GtkWidget *wid) noexcept {
  ...
  if (GDK_IS_WAYLAND_DISPLAY(pdisplay)) {
    // The GDK behavior on Wayland is not self-consistent, we must correct the display
    // coordinates to match the coordinate space used in gtk_window_move. See also
...
void Window::Show(bool show) {
  if (show) gtk_widget_show(PWidget(wid));
}
...
void Window::InvalidateRectangle(PRectangle rc) {
  if (wid) {
    const IntegerRectangle irc(rc);
    gtk_widget_queue_draw_area(PWidget(wid), irc.left, irc.top, irc.Width(), irc.Height());
  }
}
...
void Window::SetCursor(Cursor curs) {
  ...
  if (gdkCurs) UnRefCursor(gdkCurs);
}
...
PRectangle Window::GetMonitorRect(Point pt) {
  ...
  GdkDisplay *pdisplay = gtk_widget_get_display(PWidget(wid));
  GdkMonitor *monitor =
      gdk_display_get_monitor_at_point(pdisplay, pt.x + x_offset, pt.y + y_offset);
  gdk_monitor_get_geometry(monitor, &rect);
#else
  GdkScreen *screen = gtk_widget_get_screen(PWidget(wid));
  const gint monitor_num = gdk_screen_get_monitor_at_point(
      screen, static_cast<gint>(pt.x) + x_offset, static_cast<gint>(pt.y) + y_offset);
...
static void list_image_free(gpointer, gpointer value, gpointer) noexcept {
  ...
  if (list_image->pixbuf) g_object_unref(list_image->pixbuf);
  g_free(list_image);
}
...
class ListBoxX : public ListBox {
  ...
  ListBoxX() noexcept
      : widCached(nullptr), frame(nullptr), list(nullptr), scroller(nullptr),
        pixhash(nullptr), pixbuf_renderer(nullptr), renderer(nullptr),
        desiredVisibleRows(5), maxItemCharacters(0), aveCharWidth(1), delegate(nullptr) {}
  ...
  void Create(Window &parent, int ctrlID, Point location_, int lineHeight_,
              bool unicodeMode_, Technology technology_) override;
...
std::unique_ptr<ListBox> ListBox::Allocate() { return std::make_unique<ListBoxX>(); }
...
static int treeViewGetRowHeight(GtkTreeView *view) {
  ...
  gtk_tree_view_column_cell_get_size(column, nullptr, nullptr, nullptr, nullptr,
                                     &row_height);
  gtk_widget_style_get(GTK_WIDGET(view), "vertical-separator", &vertical_separator,
...
// The *override* calls are deprecated now, so only call them for older versions of GTK+.
  ...
  if (styleContext == nullptr) return;
  ...
  gtk_style_context_get_background_color(styleContext, GTK_STATE_FLAG_SELECTED,
                                         &colourBaseSelected);
  GdkRGBA colourBaseActive;
  gtk_style_context_get_background_color(styleContext, GTK_STATE_FLAG_ACTIVE,
                                         &colourBaseActive);
...
void ListBoxX::Create(Window &parent, int, Point, int, bool, Technology) {
  ...
  /* Tree and its model */
  GtkListStore *store = gtk_list_store_new(N_COLUMNS, GDK_TYPE_PIXBUF, G_TYPE_STRING);
  ...
  gtk_tree_view_column_add_attribute(column, pixbuf_renderer, "pixbuf", PIXBUF_COLUMN);

  renderer = gtk_cell_renderer_text_new();
  gtk_cell_renderer_text_set_fixed_height_from_font(GTK_CELL_RENDERER_TEXT(renderer), 1);
  gtk_tree_view_column_pack_start(column, renderer, TRUE);
  gtk_tree_view_column_add_attribute(column, renderer, "text", TEXT_COLUMN);
  ...
  g_signal_connect(G_OBJECT(widget), "button_press_event", G_CALLBACK(ButtonPress), this);
  g_signal_connect(G_OBJECT(widget), "button_release_event", G_CALLBACK(ButtonRelease),
                   this);

  GtkWidget *top = gtk_widget_get_toplevel(static_cast<GtkWidget *>(parent.GetID()));
  gtk_window_set_transient_for(GTK_WINDOW(static_cast<GtkWidget *>(wid)), GTK_WINDOW(top));
}
...
int ListBoxX::GetRowHeight() { return treeViewGetRowHeight(GTK_TREE_VIEW(list)); }
...
// returns reasonable values.
    ...
    // Unfortunately we need to be built against 3.20 to be able to support this, as it
    // requires new API.
    ...
    if (gtk_check_version(3, 20, 0) == nullptr) {
      ...
      // height on GTK 3.20 when built against an earlier version.
      border_border.top = border_border.bottom = border_border.left = border_border.right =
          1;
    }
#endif

    height = (rows * row_height + padding.top + padding.bottom + border.top +
              border.bottom + border_border.top + border_border.bottom +
              2 * gtk_container_get_border_width(GTK_CONTAINER(PWidget(list))));
#else
    height = (rows * row_height + 2 * (PWidget(frame)->style->ythickness +
    ...
    gtk_widget_style_get(PWidget(list), "horizontal-separator", &horizontal_separator,
                         nullptr);
    rc.right += horizontal_separator;
#if GTK_CHECK_VERSION(3, 0, 0)
    rc.right += (padding.left + padding.right + border.left + border.right +
...
int ListBoxX::CaretFromEdge() {
  ...
  gtk_cell_renderer_get_fixed_size(pixbuf_renderer, &renderer_width, &renderer_height);
...
static void init_pixmap(ListImage *list_image) noexcept {
  if (list_image->rgba_data) {
    ...
    if (list_image->pixbuf) g_object_unref(list_image->pixbuf);
    list_image->pixbuf = gdk_pixbuf_new_from_data(
        list_image->rgba_data->Pixels(), GDK_COLORSPACE_RGB, TRUE, 8,
        list_image->rgba_data->GetWidth(), list_image->rgba_data->GetHeight(),
        list_image->rgba_data->GetWidth() * 4, nullptr, nullptr);
  }
}
...
void ListBoxX::Append(char *s, int type) {
  ...
  if ((type >= 0) && pixhash) {
    list_image =
        static_cast<ListImage *>(g_hash_table_lookup(pixhash, GINT_TO_POINTER(type)));
  }
  GtkTreeIter iter{};
  GtkListStore *store = GTK_LIST_STORE(gtk_tree_view_get_model(GTK_TREE_VIEW(list)));
  gtk_list_store_append(GTK_LIST_STORE(store), &iter);
  if (list_image) {
    if (nullptr == list_image->pixbuf) init_pixmap(list_image);
    if (list_image->pixbuf) {
      gtk_list_store_set(GTK_LIST_STORE(store), &iter, PIXBUF_COLUMN, list_image->pixbuf,
                         TEXT_COLUMN, s, -1);
      ...
      if (pixbuf_width > renderer_width)
        gtk_cell_renderer_set_fixed_size(pixbuf_renderer, pixbuf_width, -1);
    } else {
      gtk_list_store_set(GTK_LIST_STORE(store), &iter, TEXT_COLUMN, s, -1);
    }
  } else {
    gtk_list_store_set(GTK_LIST_STORE(store), &iter, TEXT_COLUMN, s, -1);
  }
  const unsigned int len = static_cast<unsigned int>(strlen(s));
  if (maxItemCharacters < len) maxItemCharacters = len;
}

int ListBoxX::Length() {
  if (wid)
    return gtk_tree_model_iter_n_children(gtk_tree_view_get_model(GTK_TREE_VIEW(list)),
                                          nullptr);
  return 0;
}
...
#if GTK_CHECK_VERSION(3, 0, 0)
    GtkAdjustment *adj = gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(list));
#else
    GtkAdjustment *adj = gtk_tree_view_get_vadjustment(GTK_TREE_VIEW(list));
#endif
    gdouble value = (static_cast<gdouble>(n) / total) *
...
void ListBoxX::RegisterRGBA(int type, std::unique_ptr<RGBAImage> image) {
  ...
  if (list_image) {
    ...
    list_image->rgba_data = observe;
    g_hash_table_insert(pixhash, GINT_TO_POINTER(type), (gpointer)list_image);
  }
}
...
void ListBoxX::ClearRegisteredImages() { images.Clear(); }

void ListBoxX::SetDelegate(IListBoxDelegate *lbDelegate) { delegate = lbDelegate; }
...
void Menu::Destroy() noexcept {
  if (mid) g_object_unref(G_OBJECT(mid));
  mid = nullptr;
}

#if !GTK_CHECK_VERSION(3, 22, 0)
static void MenuPositionFunc(GtkMenu *, gint *x, gint *y, gboolean *,
...
void Menu::Show(Point pt, const Window &w) {
  ...
  gtk_menu_popup(widget, nullptr, nullptr, MenuPositionFunc,
                 GINT_TO_POINTER((static_cast<int>(pt.y) << 16) | static_cast<int>(pt.x)),
...
ColourRGBA Platform::ChromeHighlight() { return white; }
