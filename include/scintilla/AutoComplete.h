...
class AutoComplete {
  ...
  int widthLBDefault;
  int heightLBDefault;
  /** Ordering::PreSorted:   Assume the list is presorted; selection will fail if it is not
   * alphabetical<br /> Ordering::PerformSort: Sort the list alphabetically; start up
   * performance cost for sorting<br /> Ordering::Custom:      Handle non-alphabetical
   * entries; start up performance cost for generating a sorted lookup table
   */
  ...
  void Start(Window &parent, int ctrlID, Sci::Position position, Point location,
             Sci::Position startLen_, int lineHeight, bool unicodeMode,
             Scintilla::Technology technology, ListOptions listOptions);

  /// The stop chars are characters which, when typed, cause the auto completion list to
  /// disappear
